...
def pcn(TideSolver, wn, wn1, t, y_act, c = Constant(0.01), iter = 10, beta = 0.01, cov = np.ones((1,1)), nsteps = 1200):
  ...
  for k in ProgressBar(f'iterations').iter(range(iter)):
    ...
    y_obs_c = gauge_settwo(TideSolver, wn.copy(), wn1, t, Constant(np.exp(J)), t_trunc = 900, gauge_num = 20, nsteps = nsteps)

    c.assign(Constant(np.exp(J_hat)))

    y_obs_c_hat = gauge_settwo(TideSolver, wn.copy(), wn1, t, Constant(np.exp(J_hat)), t_trunc = 900, gauge_num = 20, nsteps = nsteps)
